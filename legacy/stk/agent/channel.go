...
func CreateDedicatedChannel(address string, port int32, A *[32]byte, localPairKeys *Keys) {
	// @todo: it should be replaced with randomness keys
	...
	go func() {
		...
		p := &CryptoPipe{}
		if encryptedEnabled {
			b := localPairKeys.private
			log.Println("KEYS", A, b)
			S = createSharedKey(A, &b)
			log.Println("SHARED KEY", S)
			p = &CryptoPipe{key: &S}
		}
		go p.in(f, wc)
		p.out(wc, f)
...
func createSharedKey(remotePublicKey *[32]byte, localPrivateKey *[32]byte) [32]byte {
