...
def provision(token, default_destination="0.0.0.0"):  # gitleaks:allow false positive - function parameter, not a secret
    """Example payload: ["user@example.com", "user", "<passcode>", "ssh-rsa <PUBLIC_KEY>"]
    """
    try:
        tokens = token.split(";")
